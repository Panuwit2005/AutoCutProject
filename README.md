# AutoCut ✂️ — ตัดคลิปอัตโนมัติ (v1.5)

แอพเดสก์ท็อปตัดคลิปอัตโนมัติ — แค่ลากวิดีโอลงไป **AI วิเคราะห์เสียงเบื้องหลัง** เลือกเฉพาะช่วงที่มีคนพูด
ตัดช่วงเงียบให้เนียน (ไม่ตัดคำขาด) แล้วบันทึกเป็นโฟลเดอร์พร้อมใช้ — **ทำงานออฟไลน์ 100%** (โมเดลฝังในตัว ไม่ต้องต่อเน็ต)

> v1.5: ไม่มีระบบซับไตเติล (เอาออกแล้ว) — แต่ใช้ **Whisper (small) ทำงานเบื้องหลัง** เพื่อจับจังหวะคำ
> ให้ตัด Dead Air แม่นขึ้น ไม่ตัดกลางคำ/ไม่ตัดก่อนพูดจบ · หน้าตา & ฟีเจอร์เหมือน v1.4

---

## ✨ ความสามารถ
- รับวิดีโอ **ทุก codec / ทุกนามสกุล** (MP4, MOV, MKV, AVI, WEBM, HEVC, …) — แปลงเป็นมาตรฐานก่อนเสมอ จึงไม่ค่อย error
- **AI เลือกช่วงที่มีคนพูด + ตัด Dead Air ระดับคำ** (Whisper small ฝังในตัว, ออฟไลน์ — ใช้แค่จังหวะคำ ไม่โชว์ซับ)
...
autocut/
  ...
  transcribe.py       Whisper (small) เบื้องหลัง → จังหวะคำ (ไม่โชว์ซับ)
  analyze.py          เลือกช่วงที่เหมาะสม + ตัด dead air ระดับคำ (มี fallback ช่วงเงียบ)
...
AutoCut **v1.5** · Python 3.11.9 · faster-whisper 1.1.0 (โมเดล small, เบื้องหลัง) · ffmpeg 8.1.1 (static) · Flask + waitress · PyInstaller 6.11.1 · pywebview · cryptography (Ed25519) — ออฟไลน์ ไม่มีซับ/ไม่มี Node
