# AutoCut ✂️ — ตัดคลิปอัตโนมัติ (v1.3)

แอพเดสก์ท็อปตัดคลิปอัตโนมัติ — แค่ลากวิดีโอลงไป AI จะถอดเสียงภาษาไทย เลือกช่วงที่พูดเด่น ๆ
ตัดช่วงเงียบทิ้ง ใส่ซับไตเติลสวย ๆ ให้ แล้วบันทึกเป็นโฟลเดอร์พร้อมใช้
(ลูกค้าไม่ต้องลง Python / Node / อะไรทั้งสิ้น ทุกอย่างฝังมาในตัว — ต่อเน็ตเฉพาะตอนทำซับพรีเมียม)

---

## ✨ ความสามารถ
- รับวิดีโอ **ทุก codec / ทุกนามสกุล** (MP4, MOV, MKV, AVI, WEBM, HEVC, …) — แปลงเป็นมาตรฐานก่อนเสมอ จึงไม่ค่อย error
- **ถอดเสียงภาษาไทย** ด้วย faster-whisper (โมเดล `medium` ฝังในตัว ~1.5GB — แม่นขึ้นมาก)
- **AI เลือกช่วงที่ดีที่สุด** ให้อัตโนมัติ (ฟุตเดิม → ได้ผลคงที่ ตัดสวยเหมือนเดิม)
- **ตัด Dead Air** (ช่วงเงียบ/ไม่พูด) — เลือกระดับ เบา / ปานกลาง / เข้ม
- **ซับไตเติลพรีเมียมด้วย HyperFrames** — 13 สไตล์ (Clip Wipe, Kinetic, Neon, Gradient, Glitch, Matrix, …) มีตัวอย่างเคลื่อนไหวตอนเอาเมาส์ชี้
- **เลือกฟอนต์ฮิตในโซเชียล 8 แบบ** (Kanit, Prompt, Mitr, Bai Jamjuree, Chakra Petch, Anuphan, Mali, Noto Sans Thai)
- **แยกไฟล์ซับ (overlay โปร่งใส .mov)** ไว้เอาไปตัดต่อเองในโปรแกรมอื่นได้
...
โหลดเฉพาะ **โค้ดที่เปลี่ยน** (ไม่กี่ KB) ไม่ต้องโหลดโมเดล ~1.5GB ใหม่ และตรวจ **ลายเซ็น Ed25519**
...
autocut/
  ...
  subtitles.py        ซับ word-level + เรนเดอร์ HyperFrames (online) → libass (สำรอง)
...
AutoCut **v1.3** · Python 3.11.9 · faster-whisper 1.1.0 (โมเดล medium) · ctranslate2 · ffmpeg 8.1.1 (static) · Node (bundled) + HyperFrames 0.6.80 (ซับพรีเมียม) · PyInstaller 6.11.1 · waitress · pywebview · cryptography (Ed25519)
